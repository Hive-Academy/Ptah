...
/**
 * Standalone version of Claude CLI detector for testing without VS Code
 */
export class ClaudeCliDetectorStandalone {
  ...
  async detectClaudeInstallation(): Promise<ClaudeInstallation | null> {
    console.log('🔍 Starting comprehensive Claude Code CLI detection...');

    try {
      // Detection strategies (excluding VS Code config)
      ...
      for (const strategy of detectionStrategies) {
        try {
          ...
          if (installation && await this.validateClaudeInstallation(installation.path)) {
            ...
            console.log(`✅ Claude CLI detected: ${installation.path} (${installation.source})`);
            return installation;
          }
        } catch (error) {
          console.warn(`Detection strategy failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }

      console.error('❌ Claude Code CLI not found in any location');
      return null;
    } catch (error) {
      console.error('Error during Claude CLI detection', error);
      return null;
    }
  }

  /**
   * Strategy 1: Check system PATH for claude command
   */
  ...
  private async detectNpmGlobalInstallation(): Promise<ClaudeInstallation | null> {
    console.log('🔍 Checking npm global installation...');
    ...
    try {
      ...
      console.warn('NPM global detection failed', error);
      return null;
    }
  }

  /**
   * Strategy 3: Check common installation locations by OS
   */
  ...
  /**
   * Strategy 4: Check user home directory installations
   */
  private async detectUserHomeInstallation(): Promise<ClaudeInstallation | null> {
    console.log('🔍 Checking user home directory...');
  ...
  /**
   * Strategy 5: Use system 'which' or 'where' commands
   */
  private async detectWithSystemCommands(): Promise<ClaudeInstallation | null> {
    console.log('🔍 Using system which/where commands...');
  ...
  private async detectVersion(claudePath: string): Promise<string> {
    try {
      ...
      console.warn('Failed to detect Claude version:', error);
    }
    return 'unknown';
  }
